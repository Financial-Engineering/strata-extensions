...
public class FxForwardTest {
	...
	private final static FxSingleTrade createFxForward(CurrencyPair pair, double amount, double rate,
			LocalDate valueDate, Period tenor) {

		HolidayCalendarId paymentCalId = HolidayCalendarId.defaultByCurrency(pair.getBase());

		LocalDate paymentDate = HolidayCalendarId.defaultByCurrency(pair.getBase()).resolve(REF_DATA)
				.nextOrSame(valueDate.plus(tenor));

		FxSingle fx = FxSingle.of(CurrencyAmount.of(pair.getCounter(), amount), FxRate.of(pair, rate), paymentDate,
				BusinessDayAdjustment.of(BusinessDayConventions.FOLLOWING, paymentCalId));
